# Sistemas Distribuidos - FIUBA

Este repositorio contiene el material de estudio, presentaciones, flashcards y exámenes integradores para la materia **Sistemas Distribuidos** de la Facultad de Ingeniería de la Universidad de Buenos Aires (FIUBA).

## 📌 Información General
* **Cursada:** 1º Cuatrimestre de 2026 (1C2026)
* **Cátedra:** Cátedra Roca
* **Recursos externos:** 
  * 📓 [Notebook Cargador con Parciales y Notas](https://notebook.google.com/notebook/0b1f5917-6726-47d1-9222-a94ae2d0630e)

---

## 📂 Estructura del Repositorio

El contenido está organizado de la siguiente manera:

* 📁 **[DIAPOSITIVAS/](file:///c:/Users/jorda/Documentos/PROYECTOS/FIUBA/Distribuidos-1C2026/DIAPOSITIVAS)**: Diapositivas oficiales de la cátedra para cada clase, abarcando temas desde Introducción y Multitasking hasta Tolerancia a Fallos, Algoritmos de Consenso y Tiempo Real.
* 📁 **[RESUMENES/](file:///c:/Users/jorda/Documentos/PROYECTOS/FIUBA/Distribuidos-1C2026/RESUMENES)**: Resúmenes de estudio detallados clase a clase en formato Markdown.
* 📁 **[PREGUNTAS FINAL/](file:///c:/Users/jorda/Documentos/PROYECTOS/FIUBA/Distribuidos-1C2026/PREGUNTAS%20FINAL)**: Preguntas y respuestas rápidas tipo tarjetas de memoria (flashcards) en formato Markdown, ideales para repasar conceptos clave clase a clase.
* 📁 **[INTEGRADORES/](file:///c:/Users/jorda/Documentos/PROYECTOS/FIUBA/Distribuidos-1C2026/INTEGRADORES)**: Enunciados de exámenes integradores de fechas anteriores (PDFs e imágenes) junto con un compilado de exámenes resueltos (`RESUELTOS.pdf`).
* 📄 **[RESUMEN INTEGRADOR.pdf](file:///c:/Users/jorda/Documentos/PROYECTOS/FIUBA/Distribuidos-1C2026/RESUMEN%20INTEGRADOR.pdf)**: Resumen y compilación de temas para el examen integrador.
* 📁 **[TP/](file:///c:/Users/jorda/Documentos/PROYECTOS/FIUBA/Distribuidos-1C2026/TP)**: Código, scripts, configuración e informe del Trabajo Práctico 1.


---

## 📖 Temario de las Clases
1. **Clase 1:** Introducción a Sistemas Distribuidos.
2. **Clase 2:** Multitasking y Comunicaciones.
3. **Clase 3:** Paralelización, Multiprocesadores y Nombres.
4. **Clase 4:** Interfaces y Protocolos.
5. **Clase 5:** Mensajes, Grupos, Middlewares y MOMs (RabbitMQ).
6. **Clase 6:** Práctica de Diseño Multicomputing.
7. **Clase 7:** Patrones de Comunicación y ZeroMQ.
8. **Clase 9:** Arquitecturas Distribuidas Simples.
9. **Clase 10:** Distribución y Coordinación de Procesos.
10. **Clase 11:** Sistemas Elásticos y de Alta Disponibilidad.
11. **Clase 13:** Data Intensive Applications (Aplicaciones de Datos Intensivos).
12. **Clase 15:** Diseño de Arquitecturas de Gran Escala.
13. **Clase 16:** Arquitecturas Orientadas a Servicios (SOA) y Cloud.
14. **Clase 17:** Tolerancia a Fallos.
15. **Clase 19:** Algoritmos de Consenso (Raft, Paxos, etc.).
16. **Clase 20:** Tiempo, Relojes, Sincronismo, Orden y Cortes de Estado.
17. **Clase 21:** Introducción a Sistemas de Tiempo Real.
